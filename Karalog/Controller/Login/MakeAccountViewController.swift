...
//                        title: "新規登録失敗", message: _err.localizedDescription, preferredStyle: .alert)
...
//                            withIdentifier: "tutorialViewController") as! UIPageViewController
//                    nextView.modalPresentationStyle = .fullScreen
//                    utility.login(
//                        first: true,
//                        user: User(
//                            name: name, goodList: [], listOrder: [], showAll: false, follow: [],
//                            follower: [], request: [], notice: [], getImage: self.getImage, id: uid)
//                    ) { _ in
//
...
//            title: "画像の送信", message: "画像認識向上のため撮影した採点画面の画像を送信することを許可しますか（後で変更可能です）",
//            preferredStyle: .alert)
//        let cancel = UIAlertAction(title: "しない", style: .cancel) { (action) in
//            self.getImage = false
//
//            Auth.auth().createUser(
//                withEmail: _mail, password: password,
...
//                        let dialog = UIAlertController(
//                            title: "新規登録失敗", message: error?.localizedDescription,
//                            preferredStyle: .alert)
//                        dialog.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
//                        self.present(dialog, animated: true, completion: nil)
//                    }
