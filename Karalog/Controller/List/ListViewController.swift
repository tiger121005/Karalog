...
//    let sortList: [String] = [
//        "追加順(遅)", "追加順(早)", "スコア順(高)", "スコア順(低)", "曲名順(早)", "曲名順(遅)", "アーティスト順(早)", "アーティスト順(遅)",
//    ]
//
//    //MARK: - UI objects
//
...
//            UINib(nibName: "MusicCell", bundle: nil), forCellWithReuseIdentifier: "musicCell")
//
//        //セクションの高さ
...
//        let delete = UIAction(
//            title: "削除", image: UIImage.trash,
//            handler: { [self] _ in
//                if self.collectionView.indexPathsForSelectedItems != nil {
//                    let indexPathList = self.collectionView.indexPathsForSelectedItems!.sorted {
//                        $1.row < $0.row
//                    }
//                    let alert = UIAlertController(
//                        title: "削除", message: String(indexPathList.count) + "個の曲のデータをリストから削除します",
//                        preferredStyle: .alert)
//                    let cancel = UIAlertAction(title: "キャンセル", style: .cancel) { (action) in
//
//                    }
//                    let delete = UIAlertAction(title: "削除", style: .destructive) {
//                        [self] (action) in
//                        idList = []
//                        for i in indexPathList {
...
//                        for id in idList {
//                            if listID == "favorite" {
//                                musicFB.favoriteUpdate(
//                                    id: id, favorite: true, completionHandler: { _ in })
//                            } else if listID == "wanna" {
//                                listFB.deleteWanna(wannaID: id)
//                            } else {
//                                musicFB.deleteMusicFromList(
//                                    selectedID: id, listID: listID, completionHandler: { _ in })
//                            }
//                        }
//
...
//                } else {
//                    let alert = UIAlertController(
//                        title: "データなし", message: "データが選択されていません", preferredStyle: .alert)
...
//                title: "リストに追加", image: UIImage.folder,
//                handler: { [self] _ in
//                    segue(identifier: .addToList)
//                })
//            let selectMenu = UIMenu(title: "", children: [delete, addToList])
//            selectBtn = UIBarButtonItem(
//                title: "",
//                image: UIImage.ellipsisCircle.withConfiguration(
...
//        editBtn.image = UIImage.ellipsisCircle.withConfiguration(
//            UIImage.SymbolConfiguration(weight: .bold))
//
//        allSelectBtn = UIBarButtonItem(
//            title: "全て選択", style: .plain, target: self, action: #selector(tapAllSelectBtn(_:)))
//
//        doneBtn = UIBarButtonItem(
//            title: "完了", style: .done, target: self, action: #selector(tapDoneBtn(_:)))
//
...
//                timeInterval: 1.0, target: self, selector: #selector(hideAlert), userInfo: nil,
//                repeats: false)
...
//                    at: IndexPath(row: i, section: 0), animated: false, scrollPosition: [])
//            }
...
//    {
//        let cell =
//            collectionView.dequeueReusableCell(withReuseIdentifier: "musicCell", for: indexPath)
//            as! MusicCell
//
...
//                } else {
//                    musicFB.deleteMusicFromList(
//                        selectedID: selectedID!, listID: self.listID,
//                        completionHandler: { _ in
//                            self.cvList.remove(at: indexPath.row)
//                            self.collectionView.deleteItems(at: [indexPath])
//                        })
...
//
//            let menu = UIMenu(
//                title: "選択", image: nil, identifier: nil, options: [], children: [addList, delete])
...
//
//            let menu = UIMenu(
//                title: "選択", image: nil, identifier: nil, options: [], children: [delete])
//            let contextMenuConfiguration = UIContextMenuConfiguration(
//                identifier: nil, previewProvider: nil
//            ) { _ in
//                menu
...
//        selectedID = cvList[indexPath.row].id!
//        musicFB.favoriteUpdate(
//            id: selectedID, favorite: cvList[indexPath.row].favorite,
//            completionHandler: { _ in
