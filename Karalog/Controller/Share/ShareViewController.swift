...
class ShareViewController: UIViewController {
    ...
    func setupBarItem() {
        ...
        magnifyingglassBtn.image = UIImage.magnifyingglass.withConfiguration(UIImage.SymbolConfiguration(weight: .bold))
    }
        ...
        Task {
            ...
            if !finalContent {
                if indexPath.row == self.shareList.count - 1 {
                    ...
                    var list = await postFB.searchPost(first: false, music: musicTF.text ?? "", artist: artistTF.text ?? "", category: category)
                    if list.isEmpty {
                        finalContent = true
                        return
                    }
