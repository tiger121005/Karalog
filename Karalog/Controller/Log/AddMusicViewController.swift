...
//        actions.append(
//            UIAction(
//                title: ModelMenuType.no.rawValue, image: nil,
//                state: self.selectedMenuType == ModelMenuType.no ? .on : .off,
//                handler: { (_) in
...
//        actions.append(
//            UIAction(
//                title: ModelMenuType.DAM.rawValue, image: nil,
//                state: self.selectedMenuType == ModelMenuType.DAM ? .on : .off,
//                handler: { (_) in
...
//        actions.append(
//            UIAction(
//                title: ModelMenuType.JOYSOUND.rawValue, image: nil,
//                state: self.selectedMenuType == ModelMenuType.JOYSOUND ? .on : .off,
//                handler: { (_) in
...
//                let view = UIView(
//                    frame: CGRect(
//                        x: CGFloat((customSlider.slider.bounds.width - 31) * CGFloat(i) / 14)
//                            + 12.5, y: customSlider.slider.frame.maxY, width: 6, height: 6))
//
//                if i <= 7 {
...
//                title: "入力された値を確認してください", message: "上の写真マークから撮影した画像を見返すことができます",
...
//            let areaHeight =
//                (tabBarController?.tabBar.frame.minY ?? viewHeight)
//                - (navigationController?.navigationBar.frame.maxY ?? 44)
//            let areaCenter =
//                (navigationController?.navigationBar.frame.maxY ?? 44) + (areaHeight / 2)
...
//                frame: CGRect(
//                    x: imageViewX, y: imageViewY, width: imageViewWidth, height: imageViewHeight))
//            imageView.image = image
//
//            closeBtn = UIButton(
//                frame: CGRect(x: viewWidth - 90, y: imageViewY - 60, width: 60, height: 60))
//            closeBtn.addAction(
//                UIAction { _ in
...
//            //日本語をパソコン言語になおす
//            let parameters = [
//                "term": (self.musicTF.text ?? "") + "　" + (self.artistTF.text ?? ""),
//                "country": "jp", "limit": "14",
//            ]
//            //termが検索キーワード　countryが国　limitが数の上限
//            //parameterを使ってsearch以降の文を書いている
//            AF.request("https://itunes.apple.com/search", parameters: parameters).responseData {
//                response in
...
//                        let iTunesData: ITunesData = try self.decoder.decode(
//                            ITunesData.self, from: data)
//
//                        guard let musicImageURL = iTunesData.results.first?.artworkUrl100 else {
...
//            postFB.post(
//                musicName: musicTF.text ?? "データなし", artistName: artistTF.text ?? "データなし",
//                musicImage: musicImage, content: content, category: category)
//        }
//    }
//
//    func add() async {
...
//
//            let filterMusic = manager.musicList.filter {
//                $0.musicName == musicTF.text ?? "" && $0.artistName == self.artistTF.text ?? ""
//            }
//            if !filterMusic.isEmpty {
//                guard let id = filterMusic.first?.id else {
//                    let alert = UIAlertController(
//                        title: "エラー", message: "エラーが発生しました", preferredStyle: .alert)
//                    let ok = UIAlertAction(title: "OK", style: .default)
//                    alert.addAction(ok)
//                    present(alert, animated: true, completion: nil)
//                    return
//                }
//                musicFB.addMusicDetail(
//                    musicID: id, time: time, score: Double(scoreTF.text ?? "") ?? 0,
//                    key: Int(keyLabel.text ?? "") ?? 0, model: selectedMenuType.rawValue,
//                    comment: textView.text ?? "", image: image)
//            } else {
//                musicFB.addMusic(
//                    musicName: musicTF.text ?? "", artistName: artistTF.text ?? "",
//                    musicImage: musicImage, time: time, score: Double(scoreTF.text ?? "") ?? 0,
//                    key: Int(keyLabel.text ?? "") ?? 0, model: selectedMenuType.rawValue,
//                    comment: textView.text ?? "", image: image,
...
//                    self.navigationController!.viewControllers[screenIndex], animated: true)
//            }
//        }
//
//    }
...
//                }
//            } else {
//                alertCtl = UIAlertController(
//                    title: "入力ミス", message: "スコアがうまく入力されていません", preferredStyle: .alert)
...
//
//            alertCtl = UIAlertController(
//                title: "入力ミス", message: "値がうまく入力されていません", preferredStyle: .alert)
//            alertCtl.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
//            present(alertCtl, animated: true, completion: nil)
//
//        }
